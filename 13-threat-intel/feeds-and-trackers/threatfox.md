...
I have an IP, a domain or a URL that looks like callback infrastructure. Is it a
published command-and-control or payload-delivery indicator, for which malware
family, and who vouched for it?
...
Malpedia link, which turn one artifact into a family name and a reading list for
[MalwareBazaar](../file-and-hash/malwarebazaar.md).

Not a history service: indicators older than six months are expired out of the
API and exports, so it cannot say what an IP did last year - that is a job for
[passive DNS](../../03-dns-and-subdomains/passive-dns/circl-pdns.md). For a
malware distribution URL use [URLhaus](urlhaus.md); for who published on the
campaign, [AlienVault OTX](alienvault-otx.md).

## Install
```bash
git clone https://github.com/abusech/ThreatFox   # optional python3 samples
# Otherwise plain HTTP POST. Free Auth-Key: https://auth.abuse.ch/
```
...
# Several artifacts at once, without one API round-trip per indicator.
curl -s https://threatfox.abuse.ch/export/csv/recent/ | grep -Ff artifacts.txt

# Browser-only pivot from a family to its IOCs; /browse/tag/<tag>/ works too:
...
Read `confidence_level` and `is_compromised` together before writing anything
down: a real 2026-08-29 row - `tappancsrendelo.hu`, payload_delivery,
js.clearfake, confidence 90, `is_compromised: True`, anonymous reporter - is a
hacked legitimate site, not attacker-owned infrastructure.
...
- **The query API hard-fails without a key**: no `Auth-Key` header returns
  ...
  2026-08-29, but the docs say otherwise, so do not build on that. The key
  portal `auth.abuse.ch` served an expired certificate that day, blocking new
  keys entirely - re-check rather than reaching for `curl -k`.
...
- **confidence_level is set by the submitter** and many rows carry reporter
  `anonymous`, so a 100 is often one person's assertion. `ip:port` entries mean
  exactly that too: matching the IP alone produces false positives on shared and
  reassigned cloud ranges, which is why the six-month expiry exists.
- **Submissions are public** and attributed to your handle unless flagged
  anonymous, and even then the indicator is published at once, telling the
  operator their infrastructure is burned. Free under fair use; commercial work
  may need the paid Spamhaus API.
...
- [AbuseIPDB](../../02-network-and-ip/reputation/abuseipdb.md) - abuse reports
