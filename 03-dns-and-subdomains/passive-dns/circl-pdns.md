...
I have a domain or an IP that came out of an incident. What did it resolve to
historically, and what else lived on that address - as seen by a European CSIRT
collecting from malware analysis and partner feeds, in a format I can merge with
other passive DNS providers.

## When to reach for it
When the infrastructure is malicious or suspected to be: a dataset fed by sandbox
output sees command-and-control domains a commercial sensor network never sees.
Look elsewhere for ordinary commercial hosting, where coverage is thin, and for
anything you need tonight: access goes only to vetted partners, while
[mnemonic-pdns.md](mnemonic-pdns.md) answers with no key at all.
...
## Output
Newline-delimited JSON in Passive DNS Common Output Format: one object per tuple,
`{rrtype, rrname, rdata, count, time_first, time_last}`, times as epoch seconds.
Metadata rides in the headers, not the body - `x-dribble-cursor` for the next
page, `x-dribble-errors` for truncation. Sort on `time_first` for the timeline;
`count` is weight, so once is a scan artefact and hundreds of times is hosting.
...
- **Field orientation is a reading trap.** Querying `circl.lu` returns `rrname` =
  185.194.93.14 and `rdata` = circl.lu, the reverse of the usual reading. Check
  which field holds your query string before automating a pivot.
...
## Alternatives
- [mnemonic-pdns.md](mnemonic-pdns.md) - no key at all, answers in one curl
- [dnsdb.md](dnsdb.md) - paid, back to 2010, the reference dataset
- [validin.md](validin.md) - free tier that adds host-response pivots
