...
It is also only a wrapper; `bin/nmap` is three lines. Run tor locally and call
nmap yourself: `proxychains4 nmap -sT -Pn -n -p <ports> <addr>.onion`.

Reach for it for the packaging - a container that boots Tor, waits for
bootstrap and proxies nmap - and only if you are willing to rebuild it. Port
state on a hidden service answers a narrow question, so do not reach here first.
...
ordinary nmap report: PORT / STATE / SERVICE and a scan summary. Read the port
table and nothing else: an open port that is not 80 or 443 is the finding, an
exposed SSH, SMTP or admin service that may correlate with a clearweb host.

## Gotchas
- **`(224.0.0.1)` is not the service's address**, just a proxychains placeholder
  from `remote_dns_subnet 224`, never the onion's IP. Nothing here unmasks it.
- **"Closed" means "not published in the descriptor".** A service answers only
  on the ports its operator mapped; infer nothing about firewall, OS or host.
...
- **Intrusive contact.** The service's Tor daemon logs the rendezvous and the
  ...
  your intent, with reporting duties - [../../LEGAL.md](../../LEGAL.md).
