...
service - that is [dark.fail](dark-fail.md), which signs its listings.

## Install
```bash
# Nothing: clearweb site, no account, no API key, no Tor client, no search API.
# The crawler is self-hostable but wants your own Tor daemon and Elasticsearch:
git clone https://github.com/ahmia/ahmia-crawler   # then venv + ./run.sh
```

## Usage
```text
https://ahmia.fi/search/?q=ransomware   # keyword search, from the browser form
https://ahmia.fi/address/       # plaintext dump of every indexed onion -
                                # 10,251 v3 addresses on 2026-08-29
https://ahmia.fi/add/onionsadded/   # undocumented recent-submissions feed -
                                    # 947 addresses on 2026-08-29
```
```bash
...
## Output
Result blocks of title, the operator's own meta description, the bare v3
hostname rendered unclickable, and a relative last-seen age ("1 week, 6 days"),
under a header giving the count and the collapsed near-duplicates. Read the age
first - it is the only liveness signal here. `/address/` and `/onions/` are flat
lists of `http://<56char>.onion/` with no metadata at all.
...
- Titles and descriptions are advertising copy, not confirmation that a service
  does anything it claims.
