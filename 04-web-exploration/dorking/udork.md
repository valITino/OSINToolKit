...
I have a domain. Can one bash script walk it through a whole set of prebuilt
Google dorks - extensions, URL strings, titles, admin panels - without me
typing each one?
...
know it is dead rather than reading a quiet run as a clean target. The author's
own replacement is GooFuzz (https://github.com/m3n0sd0n4ld/GooFuzz), which
enumerates paths through Google operators alone; for running dork sets against
one domain, use [pagodo](pagodo.md).
...
## Usage
```bash
./uDork.sh host.com -e pdf -p 3          # README example: PDFs on the target, 3 pages of results
./uDork.sh host.com -g admin             # -g runs a bundled set from ./dorks/; ./uDork.sh -l lists them
./uDork.sh host.com -g all -o report.txt # every bundled set in sequence, exported to a file
./uDork.sh host.com -f mydorks.txt -p 5  # your own list instead of the bundled ones
# All of these have returned nothing since June 2022.
```

## Output
When it worked: plain URLs streamed one per line, filtered with `grep -i` on
the domain so only in-scope hits printed, then a count line and a saved report
path under a per-target directory. Today the pipeline greps a Facebook login
page for `url?q=`, matches nothing, and prints "No results were found. Try
harder!" for every dork - which reads exactly like a clean target.
...
- **It required a live Facebook session cookie in a shell script** - full
  account credential material in plaintext on disk, with the author's own
  disclaimer that he is not responsible for what happens to the account. Do
  not resurrect the pattern with a current account.
